...
def get_cell_state_avg_embs(model,
    ...
    for possible_state in possible_states:
        ...
        avg_state_emb = mean_nonpadding_embs(state_embs, torch.Tensor(original_lens).to(device))
    ...
    for i in range(0, total_batch_length, forward_batch_size):
        ...
        if cell_states_to_model is None:
            ...
            if perturb_group == False:
                ...
                original_minibatch_lengths = torch.tensor(original_minibatch["length"], device=device)
                minibatch_lengths = torch.tensor(perturbation_minibatch["length"], device=device)
    ...
    def in_silico_perturb(self,
        ...
        if self.perturb_group == True:
            ...
            if self.cell_states_to_model is None:
                ...
                cos_sims_data = cos_sims_data.to(divice)
                ...
                if self.anchor_token is None:
                    for combo_lvl in range(self.combos+1):
                        ...
                        if self.cell_states_to_model is None:
                            ...
                            cos_sims_data = cos_sims_data.to(device)
                            for j in range(cos_sims_data.shape[0]):
                    ...
                    combo_cos_sims_data = quant_cos_sims(model,
                                                         ...
                                                         model_input_size,
                                                         self.nproc)
                    combo_cos_sims_data = combo_cos_sims_data.to(device)
