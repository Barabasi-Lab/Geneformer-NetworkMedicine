...
def get_embs(model,
    ...
    if emb_mode == 'gene':
        labelled_embs = {key:[] for key in list(labels.keys())}
    ...
    for i in trange(0, total_batch_length, forward_batch_size):
        ...
        if emb_mode == "cell":
            ...
            for counter, batch_num in enumerate(range(i, max_range)):
                for num, gene in enumerate(filtered_input_data[batch_num]["input_ids"]):
                    for key in (labels.keys()):
                        if gene in labels[key]:
                            labelled_embs[key].append(embs_i[counter][num])
        del outputs
        del minibatch
    ...
    if summary_stat is None:
        if emb_mode == 'cell':
            ...
            for key in list(labels.keys()):
                labelled_embs[key] = torch.mean(torch.stack(labelled_embs[key], dim = 0), dim = 0)
            embs_stack = labelled_embs
...
class EmbExtractor:
    ...
    def extract_embs(self, 
        ...
        self.filtered_input_data = load_and_filter(self.filter_data, self.nproc, input_data_file)
        downsampled_data = downsample_and_sort(self.filtered_input_data, self.max_ncells)
        ...
        embs = get_embs(model,
                        ...
                        self.summary_stat,
                        labels = self.emb_label)
        
        if self.summary_stat is None:
            embs_df = label_embs(embs, downsampled_data, self.emb_label, self.emb_mode)
