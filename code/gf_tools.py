...
def degree_preserving_randomization(graph, n_swaps):
    """
    ...
    -------
        randomized_graph : nx.Graph
            The degree-preserving randomized graph.
    """
    randomized_graph = graph.copy()
    nx.double_edge_swap(randomized_graph, nswap=n_swaps, max_tries=n_swaps*10)
...
def rwr(network, seed_set, restart):
    ...
    pagerank_scores = nx.pagerank(network, personalization=personalization, alpha=1-restart,max_iter=500,tol=1e-10)
...
def weight_network(weights, graph, gene_index_dict, weight_type):
    """
    ...
    weight_type : str
        attns or embeds. If embeds, weights will be rescaled
...
def separate_edges(weights1,weights2):
    """
    Try to separate two sets of edges based only on their ranked weights
    
    Parameters
    ----------
    weights1 : 1D np.ndarray or list
        The positive weights
    weights2 : 1D np.ndarray or list
        The negative weights
    
    Returns
    -------
    labs : np.ndarray
        The cumulative sum of positives at each step of the ranked list. Can be passed to auc_plot to generate a roc curve
    """
    tups1 = [(weights1[i],1) for i in range(len(weights1))]
    tups2 = [(weights2[i],0) for i in range(len(weights2))]
    tups1.extend(tups2)
    tups1 = sorted(tups1, key=operator.itemgetter(0), reverse=True)
    labs = np.array([tup[1] for tup in tups1])
    labs = np.cumsum(labs)
    return labs
    
def find_neighbors(nodes,graph):
    """
    Function to find all first neighbors of a set of nodes
    
    Parameters
    ----------
    nodes : list or set
        nodes to find neighbors of
    graph : nx.Graph
        graph from which to take neighbors
    
    Returns
    -------
        neighbors : list
            The neighbors of the input nodes, each counted only once
    """
    neighbors = []
    for node in nodes:
        ls = [n for n in graph.neighbors(node)]
        neighbors.extend(ls)
    neighbors = list(set(neighbors))
    return neighbors
             
def bin_by_hop(graph,module,n_hops):
    """
    Function to separate the nodes in a network by the length of their shortest path to a module
    
    Parameters
    ----------
    graph : nx.Graph
        the overall graph to be divided into shortest path bins
    module : nx.Graph
        the subgraph containing the module of interest (these nodes are 0 hops away from themselves)
    n_hops : the maximum shortest path length to consider
    
    Returns
    -------
        neighbors_lists : list of lists
            list of length n_hops, each entry i is a list of nodes that are i hops away from module in graph
    """
    nodes = list(module.nodes())
    neighbors_lists = [nodes]
    for i in range(n_hops):
        nodes = find_neighbors(nodes,graph)
        nodes = set(nodes)
        for j in range(i+1):
            shared = nodes.intersection(neighbors_lists[j])
            nodes = nodes - shared
        neighbors_lists.append(nodes)
    return(neighbors_lists)

def get_specific_weights(matrix,lcc_nodes, node_list,gene_index_dict):
    """
    Function to get weights between all pairs of nodes in two different modules 
    ...
    matrix : np.ndarray
        the weight matrix
    lcc_nodes : list
        the list of nodes in the first module
    node_list : list
        the list of nodes in the second module
    gene_index_dict : dict
        the dictionary that converts node ids to matrix indices
    
    Returns
    -------
        attns : list
            list of all weights between the two modules
    """
    lcc_indices = [gene_index_dict[node] for node in lcc_nodes]
    neighbor_indices = [gene_index_dict[node] for node in node_list]
    indices = list(itertools.product(lcc_indices,neighbor_indices))
    for ind in indices:
        if ind[0]==ind[1]:
            indices.remove(ind)
    attns = [matrix[ind[0],ind[1]] for ind in indices if np.isnan(matrix[ind[0],ind[1]])==False]
    return attns
